<nav class="bg-gray-900 border-b border-gray-700 shadow-lg fixed w-full top-0 left-0 z-50">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-4 py-2">
        ...
        <div class="flex items-center justify-between w-full md:w-auto">
            ...
            <button data-collapse-toggle="navbar-cta" type="button" 
                    class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-400 rounded-lg md:hidden hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-600 transition-colors" 
                    ...
                    <x-dropdown align="right" width="60">
                        <x-slot name="trigger">
                            <span class="inline-flex rounded-md">
                                <button type="button" class="inline-flex items-center px-3 py-2 border border-gray-600 text-sm leading-4 font-medium rounded-lg text-gray-300 bg-gray-800 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
            ...
            <div class="ms-3 relative">
                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                    <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" 
                            class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-500 transition hover:border-gray-400" 
                            type="button">
                ...
                <!-- Dropdown menu -->
                <div id="dropdown" class="z-10 hidden bg-gray-700 divide-y divide-gray-600 rounded-lg shadow-lg border border-gray-600 w-48">
                    <ul class="py-2 text-sm text-gray-200" aria-labelledby="dropdownDefaultButton">
                        <!-- Account Management -->
                        <li>
                            <span class="block px-4 py-2 text-xs text-gray-400">{{ __('Manejo de cuentas') }}</span>
                        </li>
                        
                        <li>
                            <a href="{{ route('profile.show') }}" class="block px-4 py-2 hover:bg-gray-600 hover:text-white transition-colors">
                                {{ __('Perfil') }}
                            </a>
                        </li>
{{--                 
                        ...
                        <li>
                            <a href="{{ route('api-tokens.index') }}" class="block px-4 py-2 hover:bg-gray-600 hover:text-white transition-colors">
                                {{ __('API Tokens') }}
                            </a>
                        </li>
                        @endif --}}
                        ...
                        <li>
                            <form method="POST" action="{{ route('logout') }}" x-data>
                                @csrf
                                <button type="submit" class="w-full text-left block px-4 py-2 hover:bg-gray-600 hover:text-white transition-colors">
                                    Salir
                                </button>
                            </form>
                        </li>
    ...
    <div id="navbar-cta" class="hidden md:hidden border-t border-gray-700 bg-gray-900">
        <div class="px-4 py-3 space-y-1">
            <!-- Mobile Teams Section -->
            @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                <div class="pb-3 border-b border-gray-700">
                    <div class="text-xs text-gray-400 mb-2">Equipo</div>
                    <div class="px-3 py-2 text-sm text-gray-300">
                        {{ Auth::user()->currentTeam->name }}
                    </div>
                    <a href="{{ route('teams.show', Auth::user()->currentTeam->id) }}" 
                       class="block px-3 py-1 text-xs text-gray-400 hover:bg-gray-700 rounded transition-colors">
            ...
            <div class="pt-3 border-t border-gray-700">
                <div class="text-xs text-gray-400 mb-2">Cuenta</div>
                <a href="{{ route('profile.show') }}" 
                   class="block px-3 py-2 text-sm text-gray-300 hover:bg-gray-700 rounded-lg transition-colors">
                    Perfil
                </a>
                {{-- @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                ...
                <form method="POST" action="{{ route('logout') }}" x-data>
                    @csrf
                    <button type="submit" 
                            class="w-full text-left px-3 py-2 text-sm text-red-400 hover:bg-gray-700 rounded-lg transition-colors">
                        Salir
                    </button>
                </form>
            </div>
